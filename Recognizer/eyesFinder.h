...
cv::Mat findEyes(cv::Mat* frame, cv::Rect* face, cv::Point centre_face, bool &eyes, int it) {
	...
	if (eyes){
		...
		*/
		...
		face->height = (face->width + 2 * face_030_020 - face_030_020*0.3);
		face->y = face->y+1.5*(face->width*0.3);
		/*cout << "face:  "<<*face << endl;
