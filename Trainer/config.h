...
#include <windows.h>
#include <conio.h>
// solo per copy_directry_no_Console()
...
#else
	inline void delete_folder(string path)
	{
...
#ifdef _WIN32
	inline void delete_folder_no_Console(string folder_to_delete) {  // delete without showing console...

																	 // ogni volta che lancio un comando con system("") vene lanciata una console window per 
																	 // il tempo di esecuzione del comando, e chiaramente se sto implementando una GUI application
																	 // oppure un servizio di background, è una cosa non gradita...


		QString path = QString::fromStdString(folder_to_delete);
