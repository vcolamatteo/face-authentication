...
{
	cv::Rect faceRect;
	if (m_facePosition != cv::Point(0, 0)) {
...
void VideoFaceDetector::detectFaceAllSizes(const cv::Mat &frame)
{
	...
	m_faceCascade->detectMultiScale(frame, m_allFaces, 1.3, 1, 0), cv::Size(frame.rows / 5, frame.rows / 5), cv::Size(frame.rows * 2 / 3, frame.rows * 2 / 3);
