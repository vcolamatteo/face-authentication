...
inline void FileLoggingITA(string Data, string Time, int type, string side) {
	...
	if (myUtils::fexists(("resources" + back_slash + "File_log" + back_slash + filelog).c_str())) {
	...
	if (type == 1) {
		...
		FILE_log << "  * (Run-Time error -- Training System Face Recognition:) Errore intercorso nell'apertura della *\n"
			...
			"  *     un applicativo terzo che ne impedisce l'apertura. Tipicamente ciò accade se si hanno    *\n"
	...
	if (type == 4) {
		FILE_log << " * *---------------* ERRORE IRREVERSIBILE: Nessun profilo utente da riconoscere! *------------* *\n"
		            " * Si sta visualizzando questo avviso perchè non risulta effettuato alcun training dei dati.    *\n"
		            " * Prima di avviare questo programma assicurarsi di avere effettuato la procedura prevista di   *\n"
		            " * addestramento. Per effettuarla lanciare il programma apposito di trainer presente in questa  *\n"
		            " * direcotry.                                                                                   *\n";
...
inline void FileLoggingENG(string Data, string Time, int type, string side) {
	...
	if (type == 4) {
		...
		FILE_log << " * *---------------------* CRITICAL ERROR: No valid user-face data found! *-------------------* *\n"
		            " * You're seeing this message because the program cannot find any valid training-data.          *\n"
		            " * Before start this program verify you're previously done the apposite training-face procedure *\n"
		            " * You can do it anytime running the apposite program you can find in the same direcotory of    *\n"
		            " * this program.                                                                                *\n";
